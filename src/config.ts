...
export function isSupportedAddress(address: string): boolean {
  return (
    /^(?:[0-9a-f]{2}:){5}[0-9a-f]{2}$/i.test(address) ||
...
function normalizeDevice(
  device: DeviceConfig,
  defaults: Omit<NormalizedDeviceConfig, "address" | "name">,
  log: Logger,
): NormalizedDeviceConfig | undefined {
  ...
  return {
    ...
    lowBatteryThreshold: boundedNumber(
...
export function normalizeConfig(
  ...
  const config = rawConfig as MiFloraConfig;
  const requestedInterval = config.fetchDataIntervalInMs;
  const fetchDataIntervalInMs = boundedNumber(
    requestedInterval,
    DEFAULT_FETCH_INTERVAL_MS,
    MIN_FETCH_INTERVAL_MS,
  ...
  if (
    ...
    log.warn(
      `fetchDataIntervalInMs was raised to the minimum of ${MIN_FETCH_INTERVAL_MS} ms`,
    );
  }

  if (config.returnDefaultDataOnError !== undefined) {
    log.warn(
      "returnDefaultDataOnError is deprecated and ignored; failed reads retain the last valid sensor values",
    );
  }
  ...
  const devicesByAddress = new Map<string, NormalizedDeviceConfig>();
...
export function deviceConfigForAddress(
