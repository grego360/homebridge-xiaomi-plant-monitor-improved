...
import type { MiFloraDevice, PlantAccessory, MiFloraConfig, MiFloraData } from "./types.js";
...
class MifloraPlatform implements DynamicPlatformPlugin {
    ...
    if (api) {
      ...
      this.api.on("didFinishLaunching", () => {
        ...
        this.run().catch((error) =>
          handleError(this.log, "Error during initial run", error)
        );
        // Set up the interval for data fetching
        setInterval(
          () =>
            this.run().catch((error) =>
              handleError(this.log, "Error during scheduled run", error)
            ),
  ...
  async searchAndAddNewPlant(): Promise<void> {
    ...
    try {
      ...
      const discoveryPromise = miflora.discover(discoveryOptions) as Promise<
        MiFloraDevice[]
      >;
      ...
      const timeoutPromise = new Promise<MiFloraDevice[]>((_, reject) => {
        setTimeout(
          () => reject(new Error("Discovery timed out after 30 seconds")),
          30000
        );
      });
        ...
        for (const deviceConfig of this.config.devices) {
          try {
            ...
            const mockDevice = {
              ...
              query: async () => {
                throw new Error(
                  "Device not available, please check Bluetooth connection"
                );
              },
            } as MiFloraDevice;
            ...
            handleError(
              this.log,
              `Error adding configured device ${deviceConfig.address}`,
              deviceError
            );
          }
        }
  ...
  async addPlantAccessory(device: MiFloraDevice): Promise<void> {
    try {
      const plant = this.plants.find(
        (plant) => plant.accessory.displayName === device.address
      );
      ...
      if (plant === undefined) {
        ...
        accessory.addService(
          ...
          `${device.address} Moisture`
        );
        accessory.addService(
          this.api.hap.Service.BatteryService,
        ...
        const indexToUpdate = this.plants.findIndex(
          (plant) => plant.accessory.displayName === device.address
        );
      ...
      handleError(
        this.log,
        `Error adding plant accessory ${device.address}`,
        error
      );
    }
  }
   ...
   */
  private async queryPlantData(plant: PlantAccessory): Promise<MiFloraData> {
    if (!plant.device) {
   ...
   */
  private updateAccessoryServices(plant: PlantAccessory, queryResult: MiFloraData): void {
  ...
  async updatePlantData(plant: PlantAccessory): Promise<void> {
    try {
      if (plant.device === undefined) {
        ...
        const indexToDelete = this.plants.findIndex(
