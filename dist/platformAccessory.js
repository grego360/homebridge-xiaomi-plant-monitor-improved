function getOrAddService(plant, api, serviceType, name, subtype) {
    return (plant.accessory.getService(serviceType) ??
        plant.accessory.addService(serviceType, name, subtype));
}
function setServiceName(service, api, name) {
    service.setCharacteristic(api.hap.Characteristic.Name, name);
}
function setLowBattery(service, api, battery, threshold) {
    service.updateCharacteristic(api.hap.Characteristic.StatusLowBattery, battery < threshold
        ? api.hap.Characteristic.StatusLowBattery.BATTERY_LEVEL_LOW
        : api.hap.Characteristic.StatusLowBattery.BATTERY_LEVEL_NORMAL);
}
export function configureAccessoryServices(plant, api, pluginVersion) {
    ...
    const moisture = getOrAddService(plant, api, api.hap.Service.HumiditySensor, `${config.name} Moisture`, `moisture-${config.address}`);
    setServiceName(moisture, api, `${config.name} Moisture`);
    const battery = getOrAddService(plant, api, api.hap.Service.Battery, `${config.name} Battery`, `battery-${config.address}`);
    setServiceName(battery, api, `${config.name} Battery`);
    configureOptionalService(plant, api, api.hap.Service.TemperatureSensor, config.displayTemperature, `${config.name} Temperature`, `temperature-${config.address}`);
    configureOptionalService(plant, api, api.hap.Service.LightSensor, config.displayLightLevel, `${config.name} Light`, `light-${config.address}`);
}
function configureOptionalService(plant, api, serviceType, enabled, name, subtype) {
...
export function updateAccessoryServices(plant, data, api) {
    ...
    const information = plant.accessory.getService(api.hap.Service.AccessoryInformation);
    information?.updateCharacteristic(api.hap.Characteristic.FirmwareRevision, firmware);
    const humidityService = plant.accessory.getService(api.hap.Service.HumiditySensor);
    if (humidityService) {
        humidityService.updateCharacteristic(api.hap.Characteristic.CurrentRelativeHumidity, Math.min(100, Math.max(0, moisture)));
    ...
    if (batteryService) {
        batteryService
            .updateCharacteristic(api.hap.Characteristic.ChargingState, api.hap.Characteristic.ChargingState.NOT_CHARGEABLE)
            .updateCharacteristic(api.hap.Characteristic.BatteryLevel, Math.min(100, Math.max(0, battery)));
    ...
    if (plant.config.displayTemperature) {
        ...
        service?.updateCharacteristic(api.hap.Characteristic.CurrentTemperature, Math.min(100, Math.max(-270, temperature)));
    }
    if (plant.config.displayLightLevel) {
        const service = plant.accessory.getService(api.hap.Service.LightSensor);
        service?.updateCharacteristic(api.hap.Characteristic.CurrentAmbientLightLevel, Math.min(100_000, Math.max(0.0001, lux)));
    }
    setAccessoryFault(plant, api, false);
}
export function setAccessoryFault(plant, api, faulted) {
    const value = faulted
...
export function accessoryContext(plant) {
    return plant.accessory.context;
}
