import { QUERY_RETRIES, QUERY_TIMEOUT_MS, } from "./settings.js";
...
export function isMiFloraData(value) {
    if (typeof value !== "object" || value === null) {
        return false;
    }
    const candidate = value;
    return (typeof candidate.firmwareInfo?.firmware === "string" &&
...
async function disconnect(device, log) {
...
export async function queryPlantData(plant, log, options = {}) {
    if (!plant.device) {
    ...
    for (let attempt = 1; attempt <= retries; attempt += 1) {
        try {
            const result = await withTimeout(readMiFloraData(plant.device), timeoutMs, `Query timed out after ${timeoutMs} ms`);
            ...
            handleError(log, `Query attempt ${attempt}/${retries} failed for ${plant.config.address}`, lastError, "warn");
            ...
            if (isUncancellableTimeout(lastError)) {
                log.warn(`Not retrying ${plant.config.address}; the timed-out BLE operation cannot be cancelled safely`);
                break;
            }
            if (attempt === retries) {
                break;
...
export async function disconnectPlantDevices(plants, log) {
...
async function updatePlantData(plant, log, api) {
    ...
    try {
        ...
        const fertilitySummary = plant.config.displayFertility
            ...
            : "";
        log.info(`${plant.config.name}: battery ${battery}%, firmware ${firmware}, temperature ${temperature}°C, light ${lux} lux, moisture ${moisture}%${fertilitySummary}`);
    }
    catch (error) {
        plant.consecutiveFailures += 1;
        context.consecutiveFailures = plant.consecutiveFailures;
